...
Wami.setup = function(options) {
	// Attaching hooks for android devices
	if (typeof WamiHooks != 'undefined') {
		Wami.startRecording = function(url, startedfn, donefn, failfn) {
			WamiHooks.startRecording(url, startedfn, donefn, failfn);
		};
		Wami.startListening = function() {
			WamiHooks.startListening();
		};
		Wami.getRecordingLevel = function() {
			return WamiHooks.getRecordingLevel();
		};
		Wami.stopRecording = function() {
			setTimeout('WamiHooks.stopRecording()', 500);
		};
		Wami.startPlaying = function(url, startedfn, donefn, failfn) {
			WamiHooks.startPlaying(url, startedfn, donefn, failfn);
		};
		Wami.getPlayingLevel = function() {
			return WamiHooks.getPlayingLevel();
		};
		Wami.stopPlaying = function() {
			WamiHooks.stopPlaying();
		};
